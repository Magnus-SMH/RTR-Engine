...
namespace RTR
{
	...
	public:
		...
		//Fetches data if its tagged with NewData
		bool Fetch() noexcept
		{
		...
		//Fetches the latest data unconditionally
		void ForceFetch() noexcept
		{
			const int back = m_Back.exchange(m_ReadSlot, std::memory_order_acq_rel);
			m_ReadSlot = back & SlotMask;
		}
